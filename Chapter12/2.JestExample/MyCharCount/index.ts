...
export class MyCharacterCounter implements ComponentFramework.StandardControl<IInputs, IOutputs> {
  ...
  public init(
    ...
    this.textbox.addEventListener(
      "keyup",
      this.GetCharacterCount.bind(this, this.maxCharacterLimit, this.textbox.value.length),
    );
    ...
    this.GetCharacterCount(this.maxCharacterLimit, this.textbox.value.length);
  }
  ...
  public updateView(context: ComponentFramework.Context<IInputs>): void {
    const changedCharCounterLimit = context.parameters.characterCounterLimit.raw ?? 0;
    if (this.maxCharacterLimit !== changedCharCounterLimit) {
      this.maxCharacterLimit = changedCharCounterLimit;
      const charRemaining = this.GetCharacterCount(this.maxCharacterLimit, this.textbox.value.length);
  ...
  /*PRIVATE FUNCTIONS*/
  /*******************/
  public GetCharacterCount(maxChar: number, currentChar: number): number {
